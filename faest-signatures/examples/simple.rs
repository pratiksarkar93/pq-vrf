...
where
    ...
    S: std::fmt::Debug + std::cmp::PartialEq + std::cmp::Eq,
{
    ...
    let signature = keypair.sign_with_rng(&mut rng1, MESSAGE.as_bytes());

    let signature2 = keypair.sign_with_rng(&mut rng2, MESSAGE.as_bytes());
    println!("Signature1: {:?}", signature);
